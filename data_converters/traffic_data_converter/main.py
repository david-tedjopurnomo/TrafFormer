...
from data_converter import DCVeritasYin
...
def main():
    ...
    # Each baseline handles the data differently. Use their corresponding 
    # data converter sub-class. 
    if arg_processor.mode == "veritasyin":
        dc = DCVeritasYin(arg_processor.input_train_path,
                          arg_processor.input_val_path,
                          arg_processor.input_test_path,
                          arg_processor.out_path)
    else:
        assert False, "Mode ''%s'' invalid." % mode
    dc.convert_and_write_data()
